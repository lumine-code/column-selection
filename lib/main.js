...
module.exports = {
  ...
  selectBox(event) {
    ...
    const { ranges } = this.boxForGesture(this.mouseStart, this.mouseEnd);
  ...
  boxForGesture(start, end) {
    ...
    this.boxCache = walked.ranges.length
      ? {
          ...
          reversed: end.column < start.column,
  ...
  moveBoxEnd(cache, endRow) {
    ...
    if (cache.ascending ? endRow > cache.endRow : endRow < cache.endRow) {
      ...
      for (
        let row = cache.endRow + step;
        cache.ascending ? row <= endRow : row >= endRow;
        row += step
      ) {
        const range = this.editor.bufferRangeForScreenRange([
          [row, cache.startColumn],
          [row, cache.endColumn],
        ]);
  ...
  computeBox(start, end) {
    ...
    for (let row = start.row; ascending ? row <= end.row : row >= end.row;) {
      ...
      const kind = this.classifyBoxRange(range, start.column, end.column, reversed);
      if (kind === "real") {
        real.ranges.push(range);
        real.rows.push(row);
      } else if (kind === "zero") {
        zero.ranges.push(range);
        zero.rows.push(row);
      }
      ascending ? row++ : row--;
    }
  ...
  // contributes nothing.
  classifyBoxRange(range, startColumn, endColumn, reversed) {
    if (!range.isEmpty()) return "real";
    ...
    if (pointStart.column !== startColumn && pointEnd.column !== endColumn) return null;
    return pointStart.column === 0 && pointEnd.column === 0 ? "zero" : "real";
  },
