...
module.exports = {
  provideBackgroundTips() {
    return {
      packageName: "column-selection",
      tips: [
        "You can keep rectangular selection on for repeated mouse gestures with {{ 'column-selection:sticky' | keystroke }}",
      ],
    };
  },
  ...
  activate() {
    ...
    this.mouseEnd = null;
    this.dragging = false;
    this.dragFrameToken = null;
    this.dragFramePending = false;
  ...
  setEditor(editor) {
    ...
    this.editorDisposable = new CompositeDisposable(
      ...
      element.onDidChangeScrollTop(didScroll),
      element.onDidChangeScrollLeft(didScroll),
    );
  },
  ...
  mouseUp(event) {
    ...
    if (this.dragging && !this.pickerFlag && this.dragFramePending) this.processDragFrame();
    this.dragging = false;
  ...
  mouseMove(event) {
    if (!this.editor || !this.dragging || this.pickerFlag) return;
    this.lastMoveEvent = event;
    this.dragFramePending = true;
    this.startDragLoop();
  ...
  startDragLoop() {
    ...
    // stops, so another event landing in that same frame joins the active loop
    // rather than starting a second one.
    if (this.dragFrameToken) return;
    this.dragFrameToken = {};
    this.scheduleDragFrame(this.dragFrameToken);
  },
  ...
  scheduleDragFrame(token) {
    window.requestAnimationFrame(() => {
      if (token !== this.dragFrameToken) return;
      if (!this.editor || !this.dragging || this.pickerFlag) {
        this.dragFrameToken = null;
        this.dragFramePending = false;
        return;
      }
      const scrolled = this.processDragFrame();
      if (scrolled || this.dragFramePending) this.scheduleDragFrame(token);
      else this.dragFrameToken = null;
    });
  },

  processDragFrame() {
    if (!this.editor || !this.dragging || this.pickerFlag || !this.lastMoveEvent) return false;

    // Match the editor's native drag pipeline: move the viewport first,
    // resolve the pointer against that new viewport second, and render the two
    // changes together. Mousemove and scroll only mark work pending, so however
    // many events arrive between frames, only the newest pointer is resolved
    // and the box is written at most once.
    let shouldSelect = this.dragFramePending;
    this.dragFramePending = false;
    const component = this.editor.component;
    const scrolled = component.autoscrollOnMouseDrag(this.lastMoveEvent);
    shouldSelect ||= scrolled || this.dragFramePending;
    this.dragFramePending = false;
    if (shouldSelect) this.selectBox(this.lastMoveEvent);
    if (scrolled) component.updateSync();
    return scrolled;
  },
  ...
  // is frozen at mousedown, the drag scheduler coalesces pointer events to one
  // update per frame, and ongoing autoscroll shifts the pointer a handful of
  // rows at a time -- so almost every frame is the previous box, a little
  // longer or a little shorter. Walking every row anyway is what made a drag
  // quadratic: frame k paid for k rows, and the loop kept the frames coming.
  // The cache keeps what the previous frame computed, so a frame that only
  // moved the end pays for the rows the pointer crossed and nothing else.
  //
